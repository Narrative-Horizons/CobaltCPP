...
namespace detail
{
	...
	{
		COBALT_FUNC_QUALIFIER static vec<4, uint, Q> call(vec<4, uint, Q> const& v, uint Mask, uint Shift)
		{
...
#	if COBALT_ARCH & COBALT_ARCH_AVX_BIT
	template<>
	COBALT_FUNC_QUALIFIER int bitCount(uint x)
	{
		return _mm_popcnt_u32(x);
	}

#	if(COBALT_MODEL == COBALT_MODEL_64)
	template<>
	COBALT_FUNC_QUALIFIER int bitCount(detail::uint64 x)
	{
