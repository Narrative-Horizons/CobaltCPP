...
#	if COBALT_CONFIG_ALIGNED_GENTYPES == COBALT_ENABLE
	...
	{
		COBALT_STATIC_ASSERT(detail::is_aligned<Q>::value, "Specialization requires aligned");

		COBALT_FUNC_QUALIFIER static mat<4, 4, float, Q> call(mat<4, 4, float, Q> const& x, mat<4, 4, float, Q> const& y)
		{
			mat<4, 4, float, Q> Result;
			cobalt_mat4_matrixCompMult(
				*static_cast<cobalt_vec4 const (*)[4]>(&x[0].data),
				*static_cast<cobalt_vec4 const (*)[4]>(&y[0].data),
				*static_cast<cobalt_vec4(*)[4]>(&Result[0].data));
			return Result;
		}
	};
#	endif
	...
	{
		COBALT_FUNC_QUALIFIER static mat<4, 4, float, Q> call(mat<4, 4, float, Q> const& m)
		{
			mat<4, 4, float, Q> Result;
			cobalt_mat4_transpose(&m[0].data, &Result[0].data);
			return Result;
		}
	};
	...
	{
		COBALT_FUNC_QUALIFIER static float call(mat<4, 4, float, Q> const& m)
		{
			return _mm_cvtss_f32(cobalt_mat4_determinant(&m[0].data));
		}
	};
	...
	struct compute_inverse<4, 4, float, Q, true>
	{
		...
		{
			mat<4, 4, float, Q> Result;
			cobalt_mat4_inverse(&m[0].data, &Result[0].data);
			return Result;
		}
	};
}//namespace detail
...
#	if COBALT_CONFIG_ALIGNED_GENTYPES == COBALT_ENABLE
	template<>
	COBALT_FUNC_QUALIFIER mat<4, 4, float, aligned_lowp> outerProduct<4, 4, float, aligned_lowp>(vec<4, float, aligned_lowp> const& c, vec<4, float, aligned_lowp> const& r)
	{
		__m128 NativeResult[4];
		cobalt_mat4_outerProduct(c.data, r.data, NativeResult);
	...
	template<>
	COBALT_FUNC_QUALIFIER mat<4, 4, float, aligned_mediump> outerProduct<4, 4, float, aligned_mediump>(vec<4, float, aligned_mediump> const& c, vec<4, float, aligned_mediump> const& r)
	{
		__m128 NativeResult[4];
		cobalt_mat4_outerProduct(c.data, r.data, NativeResult);
	...
	template<>
	COBALT_FUNC_QUALIFIER mat<4, 4, float, aligned_highp> outerProduct<4, 4, float, aligned_highp>(vec<4, float, aligned_highp> const& c, vec<4, float, aligned_highp> const& r)
	{
		__m128 NativeResult[4];
		cobalt_mat4_outerProduct(c.data, r.data, NativeResult);
