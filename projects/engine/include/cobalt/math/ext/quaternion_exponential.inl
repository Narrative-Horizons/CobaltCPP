...
namespace cobalt::math
{
	...
	COBALT_FUNC_QUALIFIER qua<T, Q> exp(qua<T, Q> const& q)
	{
		vec<3, T, Q> u(q.x, q.y, q.z);
		T const Angle = cobalt::math::length(u);
		if (Angle < epsilon<T>())
			return qua<T, Q>();
	...
	template<typename T, qualifier Q>
	COBALT_FUNC_QUALIFIER qua<T, Q> log(qua<T, Q> const& q)
	{
		vec<3, T, Q> u(q.x, q.y, q.z);
		T Vec3Len = length(u);
	...
	template<typename T, qualifier Q>
	COBALT_FUNC_QUALIFIER qua<T, Q> pow(qua<T, Q> const& x, T y)
	{
		...
		if(abs(x.w / magnitude) > cos_one_over_two<T>())
		{
			...
			if (cobalt::math::abs(VectorMagnitude - static_cast<T>(0)) < cobalt::math::epsilon<T>()) {
	...
	template<typename T, qualifier Q>
	COBALT_FUNC_QUALIFIER qua<T, Q> sqrt(qua<T, Q> const& x)
	{
